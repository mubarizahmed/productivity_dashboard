<script>
	...
	export let clientId = PUBLIC_GCAL_CLIENT_ID;
	export let scope =
	...
	function action() {
		...
		return {
			destroy() {
				...
				document.head.removeChild(gapi);
			}
		};
	}
...
<button use:action on:click={listUpcomingEvents}> List Events </button>
